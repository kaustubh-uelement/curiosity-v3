...
async function login() {
  console.log(`[Cloud Seed] Logging in to ${API_URL}/auth/login...`);
  ...
  console.log(`[Cloud Seed] Authenticated successfully as ${data.user.email} (Tenant: ${data.user.tenantId})`);
  return data.accessToken;
}

async function seedBlogs(token) {
  console.log("\n[Cloud Seed] Seeding Blog posts / Deployment updates...");
  const blogs = [
    {
  ...
  for (const blog of blogs) {
    try {
      ...
      if (res.ok) {
        ...
        console.log(`  - Blog response (${res.status}): ${txt}`);
      }
    } catch (e) {
...
async function seedPartnerships(token) {
  console.log("\n[Cloud Seed] Seeding AI Factory & Data Centre Partnerships...");
  const partners = [
  ...
  for (const partner of partners) {
    try {
      ...
      if (res.ok) {
        ...
        const txt = await res.text();
        console.log(`  - Partnership response (${res.status}): ${txt}`);
      }
    } catch (e) {
...
async function seedSolutionHighlights(token) {
  console.log("\n[Cloud Seed] Seeding Propositions / Stack Cards...");
  const stack = [
    {
      badge: "Sourcing",
  ...
  for (const item of stack) {
    try {
      ...
      if (res.ok) {
        ...
        const txt = await res.text();
        console.log(`  - Solution highlight response (${res.status}): ${txt}`);
      }
    } catch (e) {
