// ---------------------------------------------------------------------------
// Multi-Tenant CMS Client Layer
// Bridges curiosity-ai-sharp with backend services via API Gateway
// ---------------------------------------------------------------------------
...
export async function fetchActiveBlogs() {
  try {
    ...
    if (!res.ok) {
      console.warn(`[API] Failed to fetch blogs (${res.status}). Using fallback content.`);
...
export async function fetchBlogBySlug(slug) {
  try {
    ...
    console.warn(`[API] Error fetching blog slug "${slug}":`, err.message);
    return null;
  }
}
...
export async function fetchActivePartnerships() {
  try {
    ...
    if (!res.ok) {
      console.warn(`[API] Failed to fetch partnerships (${res.status}). Using fallback.`);
    ...
    console.warn("[API] Error fetching partnerships:", err.message);
    return DEFAULT_PARTNERS;
  }
}
...
export async function fetchActiveSolutionHighlights() {
  try {
    ...
    if (!res.ok) {
      console.warn(`[API] Failed to fetch solution highlights (${res.status}). Using fallback.`);
    ...
    console.warn("[API] Error fetching solution highlights:", err.message);
    return DEFAULT_STACK;
  }
}
...
/**
 * 4. CONTACT SERVICE
 * Submits lead capture and compute reservation inquiries into CMS
 */
...
export async function fetchActiveCareers() {
  try {
    ...
    if (!res.ok) {
      console.warn(`[API] Failed to fetch careers (${res.status}).`);
      return [];
    }
    ...
    console.warn("[API] Error fetching careers:", err.message);
    return [];
  }
}
...
export async function fetchActiveProductHighlights() {
  try {
    ...
    if (!res.ok) {
      console.warn(`[API] Failed to fetch product highlights (${res.status}).`);
      return [];
    }
    ...
    console.warn("[API] Error fetching product highlights:", err.message);
    return [];
  }
}
...
export async function fetchActiveShowcases(section) {
  try {
    ...
    if (!res.ok) {
      console.warn(`[API] Failed to fetch showcases (${res.status}).`);
      return [];
    }
    ...
    console.warn("[API] Error fetching showcases:", err.message);
    return [];
  }
}
...
export async function fetchActiveTestimonials() {
  try {
    ...
    if (!res.ok) {
      console.warn(`[API] Failed to fetch testimonials (${res.status}).`);
      return [];
    }
    ...
    console.warn("[API] Error fetching testimonials:", err.message);
    return [];
  }
}
